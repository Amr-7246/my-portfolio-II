...
import { useGetEntity, usePatchEntity } from '../../APIs';
...
const EditProject = () => {
  const { id } = useParams();
  const { data: projectData } = useGetEntity(`portfolio/project/${id}`);
  const { allCategories } = useGlobalContext();
  const { mutate: updateProject, isError, isPending, isSuccess } = usePatchEntity(`portfolio/project/${id}`);
  ...
  useEffect(() => {
    if (projectData) {
      ...
      setSelectedCategories(projectData.categoryId);

      // Initialize media states from existing data
      if (projectData.images?.length) {
        setImages(projectData.images.map(url => ({ localLink: url, file: null })));
      }
      if (projectData.gallery?.length) {
        setGallery(projectData.gallery.map(url => ({ localLink: url, file: null })));
      }
      if (projectData.techStack?.length) {
        setTeckStack(projectData.techStack.map(tech => ({
          name: tech.name,
          virsion: tech.version,
          iconLocalLink: tech.secure_url,
          file: null
        })));
      }
      if (projectData.videos?.length) {
        setVideo({ localLink: projectData.videos[0].url, file: null });
      }
    }
  }, [projectData]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      // Upload only new media assets that have files
      const [imagesLinks, galleryLinks, teckStackLinks, videoLinks] = await Promise.all([
        images.some(img => img.file) ? uploadAsset(images.filter(img => img.file).map(img => img.file)) : Promise.resolve([]),
        gallery.some(img => img.file) ? uploadAsset(gallery.filter(img => img.file).map(img => img.file)) : Promise.resolve([]),
        teckStack.some(tech => tech.file) ? uploadAsset(teckStack.filter(tech => tech.file).map(tech => tech.file)) : Promise.resolve([]),
        video?.file ? uploadAsset(video.file) : Promise.resolve(null)
      ]);

      // Process tech stack, combining existing and new uploads
      const processedTechStack: TechStackItem[] = teckStack.map((stack, idx) => ({
        name: stack.name,
        version: stack.virsion,
        secure_url: stack.file ? (teckStackLinks[idx]?.secure_url || '') : stack.iconLocalLink,
        publicId: stack.file ? (teckStackLinks[idx]?.publicId || '') : ''
      }));

      // Update form data with processed assets
      const updatedFormData = {
        ...formData,
        thumbnail: imagesLinks[0] || images[0]?.localLink || null,
        images: [...images.filter(img => !img.file).map(img => img.localLink), ...imagesLinks.map(link => link.secure_url)],
        gallery: [...gallery.filter(img => !img.file).map(img => img.localLink), ...galleryLinks.map(link => link.secure_url)],
        videos: videoLinks ? [{ url: videoLinks.secure_url }] : (video ? [{ url: video.localLink }] : []),
        techStack: processedTechStack,
        updatedAt: new Date().toISOString()
      };

      updateProject(updatedFormData);
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6 p-6 border-b border-[var(--border)]'>
          ...
          <div className='md:col-span-2 h-[400px] border-2 border-dashed border-[var(--border)] rounded-lg flex items-center justify-center cursor-pointer hover:border-[var(--orange)] transition-colors'
            ...
            <div className='text-center'>
              ...
              <p className='mt-2 text-[var(--text)]'>Click to update media</p>
            </div>
          </div>
        ...
        <div className='space-y-4 p-6 border-b border-[var(--border)]'>
          ...
          >
            ...
            <span className='text-[var(--text)]'>Update Tech Stack</span>
          </button>
