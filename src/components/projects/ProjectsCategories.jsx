    ...
    <div className=' w-full flex flex-col items-center border-b border-[var(--border)]'>
      {/* start Top Animated Cards */}
        ...
        <div className='relative w-full min-h-[50vh] lg:h-[60vh] border-b border-[var(--border)]'>
          <CateCard setWhichCate={setWhichCate} img={'/bg_2/photo_37_2025-07-27_07-50-15.jpg'} iAmHere={'AI'} className={`aiFullstack z-0 top-1/2 left-[25%] -rotate-12 hidden md:flex`} text={'AI fullstack'} />
          ...
          <CateCard setWhichCate={setWhichCate} img={'/bg_2/photo_6_2025-07-27_07-50-15.jpg'} iAmHere={'Front End'} className={`frontend z-[2] top-1/2 left-[75%] rotate-12 hidden md:flex`} text={'Front End'} />
        </div>
      ...
      {/* cate buttons */}
        <div className=' w-full flex flex-wrap justify-center items-center gap-5 p-5'>
          {allCate.map((cate, idx) => (
            <a href="#projects">
              <div key={idx} className={` ${WhichCate == cate ? ' shadow-[var(--shadow)] text-[var(--orange)] border-[var(--orange)] bg-transparent  ' : ''} btn `} onClick={() => setWhichCate(cate)}>
                {cate}
              </div>
            </a>
          ))}
